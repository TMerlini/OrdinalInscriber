...
export default function CommandSection({ 
  ...
  const renderStepStatus = (step: ExecutionStep, index: number) => {
    switch (step.status) {
      ...
      case StepStatus.READY:
        return (
          <div 
            className="flex-shrink-0 h-6 w-6 rounded-full bg-blue-100 flex items-center justify-center mr-3 mt-0.5 cursor-pointer"
            onClick={() => onExecuteStep(index)}
          >
            <span className="text-blue-500 text-xs font-medium">Run</span>
          </div>
        );
      default:
        return (
  ...
  return (
    <section className="p-6 border-b border-gray-200">
      <h2 className="text-xl font-medium mb-4">4. Run Commands</h2>
      
      <div className="space-y-6">
        {/* Command preview */}
        <div>
          <h3 className="font-medium text-gray-700 mb-2">Generated Commands</h3>
          <div className="bg-[#1a1a1a] text-[#f0f0f0] p-4 rounded-md shadow-inner font-mono text-sm overflow-x-auto whitespace-pre">
        ...
        {/* Execution steps */}
        <div>
          <h3 className="font-medium text-gray-700 mb-2">Execution Steps</h3>
          ...
          <ol className="space-y-4">
            <li className="flex items-start">
              ...
              <div>
                ...
                {steps[0].output && (
                  <div className="mt-2 bg-gray-100 p-3 rounded text-sm text-gray-700 font-mono">
                    {steps[0].output}
                  </div>
                )}
              </div>
