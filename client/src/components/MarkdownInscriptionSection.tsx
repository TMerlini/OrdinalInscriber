...
export default function MarkdownInscriptionSection() {
  // Single Markdown Mode
  const [markdownContent, setMarkdownContent] = useState<string>('');
  const [fileName, setFileName] = useState<string>('inscription.md');
  ...
  // Handle single markdown content change
  const handleMarkdownContentChange = (text: string) => {
  ...
  const handleGenerateCommands = (config: ConfigOptions) => {
    if (isBatchMode) {
      ...
      if (selectedItems.length === 0) {
        toast({
          title: "No markdown files selected",
          description: "Please select at least one markdown file for batch processing.",
          variant: "destructive"
        });
        return;
      }
      
      // Generate commands for each markdown file
      let batchCommands = '';
      ...
      // Generate commands for each selected markdown file
      selectedItems.forEach((item, index) => {
        ...
        // First echo the content to the file
        batchCommands += `echo '${item.content.replace(/'/g, "'\\''")}' > ${containerPath}\n`;
        
        // Command for inscription with content type
        const command = `ord wallet inscribe --fee-rate ${config.feeRate} --content-type text/markdown ${metadataOption} ${parentOption} ${destinationOption} ${satOption} --file ${containerPath}`;
        
        batchCommands += `echo 'Processing ${item.name}...'\n`;
        batchCommands += `${command}\n\n`;
        
        steps.push({
          description: `Save content for ${item.name}`,
          command: `echo '...' > ${containerPath}`,
          status: StepStatus.PENDING
        });
        
        steps.push({
          description: `Inscribe ${item.name}`,
          command,
          status: StepStatus.PENDING
        });
      });
      ...
      setCommands(`echo '${markdownContent.replace(/'/g, "'\\''")}' > ${containerPath}\n${command}`);
      setExecutionSteps([
        {
          description: `Save markdown content to file`,
          command: `echo '...' > ${containerPath}`,
          status: StepStatus.PENDING
        },
        {
          description: `Inscribe markdown file`,
          command,
          status: StepStatus.PENDING
        }
      ]);
  ...
  const handleStartProcessing = () => {
    ...
    toast({
      ...
      description: "Processing markdown inscriptions...",
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center mb-4">
        <h2 className="text-2xl font-bold">Markdown Inscription</h2>
      ...
      <Alert className="bg-blue-50 dark:bg-blue-900/20 border-blue-300 dark:border-blue-800">
        ...
        <AlertTitle>About Markdown Inscriptions</AlertTitle>
        <AlertDescription>
          Markdown inscriptions allow you to permanently store formatted text content on the Bitcoin blockchain.
          Markdown supports rich formatting including headings, lists, links, and code blocks.
          The content will be inscribed with the MIME type "text/markdown" which enables proper rendering.
